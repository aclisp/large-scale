        ...
        LRAD_TOKEN operator, CONF_SECTION *parent)
{
    ...
    cp->attr = rad_strdup(attr);
    cp->value = rad_strdup(value);
 ...
 *  Free a CONF_PAIR
 */
static void cf_pair_free(CONF_PAIR **cp)
{
    if (!cp || !*cp) return;
        ...
        CONF_SECTION *parent)
{
    ...
    cs->name1 = rad_strdup(name1);
    cs->name2 = (name2 && *name2) ? rad_strdup(name2) : NULL;
                       ...
                       char *output, const char *input)
{
    ...
    while (*ptr) {
        ...
        if ((*ptr == '$') && (ptr[1] == '{')) {
            ...
            if (end == NULL) {
                *p = '\0';
                radlog(L_ERROR, "[config] %s(%d): Variable expansion missing '}'",
                       cf, *lineno);
                return NULL;
            }
            ...
            while (cp == NULL) {
                ...
                *q = '\0';
                ...
                if (*ptr == '.') {
                    ...
                    if (next == NULL) {
                        radlog(L_ERROR, "[config] %s(%d): No such section %s in variable %s",
                               cf, *lineno, name, input);
                        return NULL;
                    }
                    cs = next;
                    ...
                    if (cp == NULL) {
                        radlog(L_ERROR, "[config] %s(%d): No such entry %s for string %s",
                               cf, *lineno, name, input);
                        return NULL;
                    }
                }
            ...
            if (end == NULL) {
                *p = '\0';
                radlog(L_ERROR, "[config] %s(%d): Environment variable expansion missing '}'",
                       cf, *lineno);
                return NULL;
            }
...
/*          } */
        ...
        case PW_TYPE_BOOLEAN:
            ...
            if ((strcasecmp(value, "yes") == 0) ||
                ...
                *(int *)data = 0;
                radlog(L_ERROR, "[config parser] Bad value \"%s\" for boolean variable %s",
                       value, variables[i].name);
                return -1;
            }
            DEBUG2("[config parser] %s: %s = %s",
                    cs ? cs->name1 : "unknown",
                    variables[i].name,
                    value);
            break;
        ...
        case PW_TYPE_STRING_PTR:
             ...
             *  been expanded.
             */
            if (cs && value && (value == variables[i].dflt)) {
        ...
        case PW_TYPE_IPADDR:
            ...
            if (ipaddr == 0) {
                radlog(L_ERROR, "[config parser] Can't find IP address for host %s", value);
                return -1;
            }
            DEBUG2("[config parser] %s: %s = %s IP address (%s)",
                    cs ? cs->name1 : "unknown",
                    variables[i].name,
        ...
        default:
            radlog(L_ERROR, "[config parser] Type %d not supported yet", variables[i].type);
            return -1;
            break;
                     ...
                     CONF_SECTION *parent)
{
    ...
    if ((name1 != NULL) && (name1[0] == '_')) {
        radlog(L_ERROR, "[config] %s(%d): Illegal configuration section name",
               cf, *lineno);
        return NULL;
    }
    ...
    for (;;) {
        ...
        if ((len == sizeof(buf)) &&
            (cbuf[len - 1] != '\n')) {
            radlog(L_ERROR, "[config] %s(%d): Line too long",
                   cf, *lineno);
            cf_section_free(&cs);
            return NULL;
        }
        ...
        if (strcasecmp(buf1, "$INCLUDE") == 0) {
            ...
            DEBUG2( "[config] %s(%d): Including file: %s", cf, *lineno, value );
        ...
        if (t1 == T_RCBRACE) {
            if (name1 == NULL || buf2[0]) {
                radlog(L_ERROR, "[config] %s(%d): Unexpected end of section",
                       cf, *lineno);
                cf_section_free(&cs);
                return NULL;
            }
        ...
        if (buf1[0] != 0 && buf2[0] == 0 && buf3[0] == 0) {
            ...
            radlog(L_ERROR, "[config] %s(%d): Line is not in 'attribute = value' format",
                   cf, *lineno);
            cf_section_free(&cs);
            return NULL;
        }
         ...
         */
        if (buf1[0] == '_') {
            radlog(L_ERROR, "[config] %s(%d): Illegal configuration pair name \"%s\"",
                   cf, *lineno, buf1);
            cf_section_free(&cs);
            return NULL;
        }
     ...
     */
    if (name1 != NULL) {
        radlog(L_ERROR, "[config] %s(%d): Unexpected end of file", cf, *lineno);
        cf_section_free(&cs);
        return NULL;
    }
            ...
            const char *conffile, CONF_SECTION *parent)
{
    ...
    if ((fp = fopen(conffile, "r")) == NULL) {
        if (fromfile) {
            radlog(L_ERROR|L_CONS, "[config] %s(%d): Unable to open file \"%s\": %s",
                    fromfile, fromline, conffile, strerror(errno));
        } else {
            radlog(L_ERROR|L_CONS, "[config] Unable to open file \"%s\": %s",
...
int dump_config_section(const CONF_SECTION *cs, int indent)
{
    ...
    for (ci = cs->children; ci; ci = ci->next) {
        if (ci->type == CONF_ITEM_PAIR) {
            cp=cf_itemtopair(ci);
            DEBUG("[config dump] %.*s%s = %s",
                indent, "\t\t\t\t\t\t\t\t\t\t\t",
                cp->attr, cp->value);
        } else {
            scs=cf_itemtosection(ci);
            DEBUG("[config dump] %.*s%s %s%s{",
                indent, "\t\t\t\t\t\t\t\t\t\t\t",
                scs->name1,
                scs->name2 ? scs->name2 : "",
                scs->name2 ?  " " : "");
